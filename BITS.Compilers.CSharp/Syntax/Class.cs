...
{
    public sealed class Class : Class<Class>
    {
        # region • Ctor's •
        ...
        internal Class(ClassDeclarationSyntax @syntaxNode)
            : base(@syntaxNode) {}
        ...
        protected override Class New(ClassDeclarationSyntax @syntaxNode)
        {
            return new Class(@syntaxNode);
        }

        # endregion Methods
    }

    public abstract class Class<TSelf> : Type<TSelf, ClassDeclarationSyntax>
        where TSelf : Class<TSelf>
    {
        # region • Ctor's •

        protected Class(String @identifier)
        {
            this.Init(Factory(@identifier));
        }

        internal protected Class(ClassDeclarationSyntax @syntaxNode)
            : base(@syntaxNode) {}
        ...
        public virtual TSelf WithBaseClass(String @typeName, params Object[] @params)
        {
            if (@params != null)
                @typeName = String.Format(@typeName, @params);

            return Self(this.Syntax
                .WithBaseClass(@typeName));
        }

        public virtual TSelf WithBaseClass(BaseTypeSyntax @classTypeSyntax)
        {
            return Self(this.Syntax
                .WithBaseClass(@classTypeSyntax));
        }

        public virtual TSelf WithBaseClass<T>(Class<T> @class)
            where T : Class<T>
        {
            return Self(this.Syntax
                .WithBaseClass(@class.Syntax.GetSimpleBaseType()));
        }
        ...
        public virtual TSelf AddMembers<T, TMember>(IEnumerable<T> @members)
            where T : Member<T, TMember>
            where TMember : MemberDeclarationSyntax
        {
            return this.AddMembers(@members.ToArray());
        }

        public virtual TSelf AddMembers<T, TMember>(params Member<T, TMember>[] @members)
            where T : Member<T, TMember>
            where TMember : MemberDeclarationSyntax
        {
        ...
        protected virtual TSelf AddMembers(params MemberDeclarationSyntax[] @membersDeclarationSyntax)
        {
            return Self(this.Syntax
        ...
        private static ClassDeclarationSyntax Factory(String @identifier)
        {
